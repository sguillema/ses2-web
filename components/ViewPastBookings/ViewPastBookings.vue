...
export default {
  ...
  data() {
    return {
      ...
      subHeaders: [{ text: 'Description', value: 'title', sortable: false }],
      sessions: [],
      bookings: [],
      workshops: [],
      bookingDetails: [],
  ...
  async mounted() {
    ...
    for (let i = 0; i < bookingsWithData.length; i++) {
      ...
      if (moment().isAfter(bookingWithData.session.startTime)) {
        pastBookings.push(bookingWithData)
      }
    }
